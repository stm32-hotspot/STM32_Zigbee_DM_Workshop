...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	switch (GPIO_Pin) {
	case BUTTON_SW1_PIN:
		APP_ZIGBEE_LaunchPushButtonTask(APP_ZIGBEE_PUSH_BUTTON_TASK_SW1);
      break;

    case BUTTON_SW2_PIN:
      APP_ZIGBEE_LaunchPushButtonTask(APP_ZIGBEE_PUSH_BUTTON_TASK_SW2);
      break;
    ...
    case BUTTON_SW3_PIN:
      ...
      if(HAL_GPIO_ReadPin(BUTTON_SW3_GPIO_PORT, BUTTON_SW3_PIN))
      {
        ...
        if((press_duration < GPIO_SW_LONG_PRESS_DURATION) && (press_duration > GPIO_SW_MIN_PRESS_DURATION))
        {
          /* SW3 short press detected */
          APP_DBG("SW3 short press, z3.0 commissioning");
          APP_ZIGBEE_LaunchPushButtonTask(APP_ZIGBEE_PUSH_BUTTON_TASK_SW3_SHORT);
        }
      }
      else
      {
...
static void SW3_LONG_PRESS_TIMER_CB(void)
{
  /* SW3 long press detected */
	...
	APP_ZIGBEE_LaunchPushButtonTask(APP_ZIGBEE_PUSH_BUTTON_TASK_SW3_LONG);
	uint32_t press_duration = abs(HAL_GetTick() - ButtonPressTime);
	APP_DBG("SW3 press duration  %d",press_duration);
